"""
runner.py  —  CAIT Daily Runner  (port 5562)
...
Start : python scripts/runner.py
URL   : http://localhost:5562

New handles added to the COMMENTS sheet appear automatically —
the queue refreshes every 30 seconds, or click the Refresh button.
"""

import os, json, time, datetime, threading, webbrowser
import gspread
...
POST_OPT   = os.path.join(OUT, "post_options.json")
...
def api_reload_cq():
    """Re-read COMMENTS sheet — picks up new handles added since startup."""
    global cq, cc, ws_c
    try:
...
var VQ=[], VDone=new Set(), VIdx=0;
var poll=null;

// ── boot ──────────────────────────────────────────────────────────────────────
var VQLoaded=false;
async function boot(){
  // Load Comments immediately — show UI right away
  ...
  // Load Validation in background (737 accounts = 134KB, slow)
  fetch('/api/vq').then(function(r){return r.json()}).then(function(vr){
    VQ=vr; VQLoaded=true;
  }).catch(function(){});
}

// ── refresh queue (picks up new handles from sheet) ───────────────────────────
async function refreshQueue(){
  try{
    var r=await fetch('/api/reload-cq').then(function(x){return x.json()});
    if(!r.ok){return}
    // Merge new items — keep existing CDone and COrigs
    var existingRows=new Set(CQ.map(function(x){return x.row}));
    var newItems=r.count; // server already updated cq, re-fetch
    ...
    cr.forEach(function(x){
      if(!existingRows.has(x.row)){
        ...
        // Update existing (comment may have been edited)
...
function goTab(t){
  ...
  if(!VQLoaded){
    document.getElementById('app').innerHTML=
      '<div class="card done-card"><p style="color:#888">Loading validation accounts...</p></div>';
